...
spinButton.addEventListener("click", async () => {
  ...
  result.setSpinning();
  try {
    const values = await Promise.all([wheel1.spin(), wheel2.spin(), wheel3.spin()]);

    const maximumCount = getMaxCount(values);
    ...
    console.log('Something is wrong with the machine');
  }
